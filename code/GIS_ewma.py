...
def GIS_ewma_ridge_torch(X, alpha, ridge, assume_centered=False, verbose=False):
    n, p = X.shape
    ...
    deltaGIS = ridge + deltaGIS
    sigmahat = (u @ torch.diag(deltaGIS) @ u.T.conj()).real
    return sigmahat


def GIS_ewma_prec_ridge_torch(X, alpha, ridge, assume_centered=False, verbose=False):
    n, p = X.shape
