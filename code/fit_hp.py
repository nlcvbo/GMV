...
from markowitz import GMV_P_torch
...
class EWMA_model(torch.nn.Module):
    ...
    def forward(self, Y):
        n, p = Y.shape
        res_s = torch.zeros(Y.shape[0]//20-self.lag)
        res_e = torch.zeros(Y.shape[0]//20-self.lag)
        for i in range(self.lag, Y.shape[0]//20):
            ...
            if self.estimator == 'LWO':
                Sigma = weighted_LWO_estimator.wLWO_estimator_torch(Y_train, torch.diag(W/Y_train.shape[0]), assume_centered = False)
            ...
            if P is None:
                e, s, IC = GMV_P_torch(Y_test, Sigma, wb=torch.ones(p)/p)
            else:
...
if __name__ == "__main__":
    ...
    model, best_a, loss = minimization(Y, a = 0.96, lag = 24, n_epochs = 10, lr = 1e-2, estimator = 'LWO', verbose = True)
    print("Best a =", best_a)

    # a_tab = list(np.linspace(0.9,1.1,40))
    # plot_vol(Y, a_tab, lag = 24, estimator_list = ['LWO', 'S', 'GIS', 'ANS'])