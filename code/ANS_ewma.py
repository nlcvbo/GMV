...
def analytical_shrinkage_ewma_torch(X, alpha, assume_centered = False):
    ...
    H = h*L.T
    x = (L - L.T)/H
    ftilde = (3/4/np.sqrt(5))*torch.mean(torch.maximum(1-x**2/5,torch.zeros(x.shape, dtype=x.dtype, requires_grad=x.requires_grad))/H,axis=1)
    Hftemp = (-3/10/np.pi)*x + (3/4/np.sqrt(5)/np.pi)*(1-x**2/5)*torch.log(torch.abs((np.sqrt(5)-x)/(np.sqrt(5)+x)))
    Hftemp[torch.abs(x) == np.sqrt(5)] = (-3/10/np.pi)*x[torch.abs(x) == np.sqrt(5)]
    Hftilde = (Hftemp/H).mean(axis=1)
    if p <= n:        
        m = np.pi*Hftilde + np.pi*ftilde*1j
        c = p/n
        ...
        # TODO
        Hftilde0 = (1/np.pi)*(3/10/h**2+3/4/np.sqrt(5)/h*(1-1/5/h**2)*torch.log((1+np.sqrt(5)*h)/(1-np.sqrt(5)*h)))*(1/lambda_).mean(axis=0)
        dtilde0 = 1/(np.pi*(p-n)/n*Hftilde0)
        dtilde1 = lambda_/(np.pi**2*lambda_**2*(ftilde**2+Hftilde**2))
        dtilde = torch.concatenate([dtilde0*torch.ones((p-n), dtype=x.dtype, requires_grad=x.requires_grad), dtilde1], axis=0)
...
def analytical_shrinkage_prec_ewma_torch(X, alpha, assume_centered = False):
    ...
    ftilde = (3/4/np.sqrt(5))*torch.mean(torch.maximum(1-x**2/5,torch.zeros(x.shape, dtype=x.dtype, requires_grad=x.requires_grad))/H,axis=1)
    Hftemp = (-3/10/np.pi)*x + (3/4/np.sqrt(5)/np.pi)*(1-x**2/5)*torch.log(torch.abs((np.sqrt(5)-x)/(np.sqrt(5)+x)))
    Hftemp[torch.abs(torch.abs(x) - np.sqrt(5)) < eps] = (-3/10/np.pi)*x[torch.abs(torch.abs(x) - np.sqrt(5)) < eps]
    Hftilde = (Hftemp/H).mean(axis=1)
    if p <= n:        
        m = np.pi*Hftilde + np.pi*ftilde*1j
        c = p/n
    ...
    dtilde[dtilde < x] = x
    dtilde[max(0,p-n):p] = torch.sort(dtilde[max(0,p-n):p], descending=True)[0]
