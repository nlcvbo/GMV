...
def LIS_ewma(X, alpha, assume_centered=False, verbose=False):
    ...
    if p <= n:  # case where sample covariance matrix is not singular
        ...
        deltahat_1 = np.concatenate(
            (delta, (1 - c) * invlambda + 2 * c * invlambda * theta), axis=None
        )
...
def LIS_ewma_prec_torch(X, alpha, assume_centered=False, verbose=False):
    ...
    (Lj**2 * h / (Lj_i**2 + Lj**2 * h**2)).mean(axis=0)  # its conjugate
    ...
    deltaLIS_1[max(0, p - n) : p] = torch.sort(
        deltaLIS_1[max(0, p - n) : p], descending=True
    )[0]
    psihat = (u @ torch.diag(deltaLIS_1) @ u.T.conj()).real
    return psihat


def LIS_ewma_ridge_torch(X, alpha, ridge, assume_centered=False, verbose=False):
    n, p = X.shape
    ...
    deltaLIS_1, _ = torch.sort(deltaLIS_1, descending=False)
    deltaLIS_1 = ridge + deltaLIS_1
    sigmahat = (u @ torch.diag(deltaLIS_1) @ u.T.conj()).real
    return sigmahat


def LIS_ewma_prec_ridge_torch(X, alpha, ridge, assume_centered=False, verbose=False):
    n, p = X.shape
    ...
    deltaLIS_1 = 1 / (ridge + 1 / deltaLIS_1)
    psihat = (u @ torch.diag(deltaLIS_1) @ u.T.conj()).real
    return psihat
