...
def wLWO_estimator_oracle(X, w, cov, assume_centered=False, S_r=None, est=False):
    ...
    beta2_opt = (
        -np.trace(S @ cov.T) / p * np.trace(S @ S_r.T) / p
        + S2 / p * np.trace(S_r @ cov.T) / p
    )
...
def wLWO_estimator_unknown_mean(X, w, S_r=None, est=False):
    ...
    C22 = (
        ...
        - 3 * (w**2).sum() * (w**4).sum()
        + 2 * (w**6).sum()
        + (w**2).sum() * (2 * w**2 * (1 - w) ** 2).sum()
        - 2 * (w**4 * (1 - w) ** 2).sum()
    )
    C33 = (
        (w**2).sum() * (4 * w**2 * (1 - w) ** 2).sum()
        - (4 * w**4 * (1 - w) ** 2).sum()
        + 2 * (w**2).sum() ** 3
        - 6 * (w**2).sum() * (w**4).sum()
        + 4 * (w**6).sum()
    ...
    q3 = (
        -4 * (w**2).sum() * (w**2 * (1 - w)).sum()
        + 4 * (w**4 * (1 - w)).sum()
        + 4 * (w**3).sum() * (w**2 * (1 - w)).sum()
        + (w**2).sum() * (4 * w**3 * (1 - w)).sum()
        - 8 * (w**5 * (1 - w)).sum()
    )
    q4 = (
        2 * (w * (1 - 2 * w) ** 2 * ((w**2).sum() - w**2)).sum()
        - 2 * (w**3 * (1 - 2 * w) ** 2).sum()
        - 2 * (w**2 * (1 - 2 * w) ** 2 * ((w**2).sum() - 2 * w**2)).sum()
    )
    ...
    E22 = (
        ...
        + (w * (1 - w) ** 2).sum() ** 2
        + (w**3).sum() ** 2
        + 2 * (w * ((1 - w) ** 2 + w**2) * ((w**2).sum() - w**2 - (w**3).sum())).sum()
        - (
            w**2 * (1 - w) ** 4
            + w**6
            + 2 * w**2 * (w**2 + (1 - w) ** 2) * ((w**2).sum() - 2 * w**2)
        ).sum()
    )
    E33 = (
        2 * q2
        + 4 * (w**2 * (1 - w)).sum() ** 2
        - 4 * (w**4 * (1 - w) ** 2).sum()
        - 8 * (w**2 * (1 - w) * ((w**2).sum() - w**2 - (w**3).sum())).sum()
        + 8 * (w**3 * (1 - w) * ((w**2).sum() - 2 * w**2)).sum()
    )
...
def wLWO_estimator_unknown_mean_torch(X, w, S_r=None, est=False):
    ...
    C11 = (w**2 * (1 - w) ** 4 - w**6).sum(axis=0) + (w**2).sum(axis=0) * (
        w**4
    ).sum(axis=0)
    C22 = (
        (w**2).sum(axis=0) ** 3
        - 3 * (w**2).sum(axis=0) * (w**4).sum(axis=0)
        + 2 * (w**6).sum(axis=0)
        + (w**2).sum(axis=0) * (2 * w**2 * (1 - w) ** 2).sum(axis=0)
        - 2 * (w**4 * (1 - w) ** 2).sum(axis=0)
    )
    C33 = (
        (w**2).sum(axis=0) * (4 * w**2 * (1 - w) ** 2).sum(axis=0)
        - (4 * w**4 * (1 - w) ** 2).sum(axis=0)
        + 2 * (w**2).sum(axis=0) ** 3
        - 6 * (w**2).sum(axis=0) * (w**4).sum(axis=0)
        + 4 * (w**6).sum(axis=0)
    )
    ...
    q2 = (
        (w**2).sum(axis=0) ** 2
        - 4 * (w**3).sum(axis=0) * (w**2).sum(axis=0)
        - (w**4).sum(axis=0)
        + 4 * (w**5).sum(axis=0)
        + 2 * (w**3).sum(axis=0) ** 2
        - (
            (w**2).sum(axis=0) ** 2 * w**2
            - 4 * w**4 * (w**2).sum(axis=0)
            - w**2 * (w**4).sum(axis=0)
            + 6 * w**6
        ).sum(axis=0)
    )
    q3 = (
        -4 * (w**2).sum(axis=0) * (w**2 * (1 - w)).sum(axis=0)
        + 4 * (w**4 * (1 - w)).sum(axis=0)
        + 4 * (w**3).sum(axis=0) * (w**2 * (1 - w)).sum(axis=0)
        + (w**2).sum(axis=0) * (4 * w**3 * (1 - w)).sum(axis=0)
        - 8 * (w**5 * (1 - w)).sum(axis=0)
    )
    q4 = (
        2 * (w * (1 - 2 * w) ** 2 * ((w**2).sum(axis=0) - w**2)).sum(axis=0)
        - 2 * (w**3 * (1 - 2 * w) ** 2).sum(axis=0)
        - 2 * (w**2 * (1 - 2 * w) ** 2 * ((w**2).sum(axis=0) - 2 * w**2)).sum(axis=0)
    )

    D11 = (
        2 * (w**3 * (1 - w) ** 2).sum(axis=0)
        - 2 * (w**4 * (1 - w) ** 2).sum(axis=0)
        + (w**4).sum(axis=0)
        - 2 * (w**5).sum(axis=0)
        - (w**2).sum(axis=0) * (w**4).sum(axis=0)
        + 2 * (w**6).sum(axis=0)
    )
    D22 = q1 + q2 + q3
    D33 = (
        ...
        + q4
        + (w**3).sum(axis=0) ** 2
        + 2 * (w**2 * (1 - w)).sum(axis=0) ** 2
        + (w * (1 - w) ** 2).sum(axis=0) ** 2
        - (w**2 * (w**2 + (1 - w) ** 2) ** 2).sum(axis=0)
    )
    ...
    E22 = (
        ...
        + 2
        * (
            w * ((1 - w) ** 2 + w**2) * ((w**2).sum(axis=0) - w**2 - (w**3).sum(axis=0))
        ).sum(axis=0)
        - (
            w**2 * (1 - w) ** 4
            + w**6
            + 2 * w**2 * (w**2 + (1 - w) ** 2) * ((w**2).sum(axis=0) - 2 * w**2)
        ).sum(axis=0)
    )
    E33 = (
        2 * q2
        + 4 * (w**2 * (1 - w)).sum(axis=0) ** 2
        - 4 * (w**4 * (1 - w) ** 2).sum(axis=0)
        - 8
        * (
            w**2 * (1 - w) * ((w**2).sum(axis=0) - w**2 - (w**3).sum(axis=0))
        ).sum(axis=0)
        + 8 * (w**3 * (1 - w) * ((w**2).sum(axis=0) - 2 * w**2)).sum(axis=0)
    )
